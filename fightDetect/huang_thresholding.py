...
class HuangThresholding:
    def __init__(self, data, upper_limit):
        self.data = data
    ...
    def find_bin_limits(self):
        ...
        """
        first_bin = next(i for i, x in enumerate(self.data[:self.upper_limit-1]) if x != 0)
        last_bin = next(i for i, x in enumerate(reversed(self.data[:self.upper_limit])) if x != 0)

        return first_bin, self.upper_limit - 1 - last_bin

    def calculate_mu(self):
        """
        Calculate mu_0 and mu_1.
        """
        mu_0 = np.zeros(self.upper_limit-1)
        num_pix = sum_pix = 0.0
        for ih in range(self.first_bin, self.upper_limit-1):
            sum_pix += ih * self.data[ih]
            num_pix += self.data[ih]
            mu_0[ih] = sum_pix / num_pix

        mu_1 = np.zeros(self.upper_limit-1)
        num_pix = sum_pix = 0.0
    ...
    def calculate_entropy(self, it):
        ...
        for ih in range(it + 1, self.upper_limit-1):
    ...
    def find_threshold(self):
        ...
        threshold = -1
        min_ent = float("inf")
        for it in range(self.upper_limit-1):
